...
globals [tortoises-to-add-to-group]
; Global variables:
...
to setup
  ...
  ; Set patch colors based on their y-coordinate to separate visually the groups
  ask patches
  [
    set pcolor ifelse-value (pycor = -5)
      [red]
      [ifelse-value (pycor = 5)
        [blue]
        [black]
      ]
  ]
  ...
  let spacing (80 / (number-of-tortoises + 1))
  ...
  [
    ...
    set group-id one-of [0 1 2]
    set heading one-of [0 180]

    setxy start-xcor -10 + (group-id * 10)
...
to go
  ...
  ; Has the goal been met for both types of agents? (When groups see which tortoises to expel, they don't have to expel any and turtles can remain in-group).
  if (all? groups [stable?] and all? tortoises [in-group])
  [stop]
...
to move-tortoises
  ...
  [
    ...
    [
      ...
      forward 1
      if member? ycor [-10 0 10]
      [
        ; Join group
        set group-id (ycor + 10) / 10
        set heading one-of [0 180]
        set tortoises-to-add-to-group lput self tortoises-to-add-to-group
      ]
    ]
