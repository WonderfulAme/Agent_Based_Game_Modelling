; Define the breed "tortoises" to represent individual tortoise agents
...
; Define attributes specific to the "groups" breed
...
globals [next-group-id            ; The next available group ID to be assigned
         new-groups                ; A list that stores groups created when tortoises form new groups
         tortoises-to-add-to-group] ; A list of tortoises to be added to an existing group
...
; Procedure for leader/participant movement
to move-leader-participant
  set heading heading + one-of (range -5 5)
  forward 1
  move-to-group
  move-to-agent
end

; Procedure for follower movement
to move-follower
  set heading heading + one-of (range -5 5)
  forward 0.3
  move-to-group
...
; Procedure for moving towards a group
to move-to-group
  ; Target will be closest group within vision range
  ...
  if target != nobody [
    ...
    if (color != [group-color] of target) and (distance target <= vision-field / 3) [
...
; Procedure for moving towards another tortoise
to move-to-agent
    ; Target will be closest agent within vision range
  let target min-one-of other tortoises in-cone vision-field 120 [distance myself]
  if target != nobody [
    face target               ; Turn towards the target tortoise
    forward 1                 ; Move forward by 1 unit

    ; If color mismatch with target tortoise and within close range
    if (color != [color] of target) and (distance target <= vision-field / 3) [
      set heading heading + 45 + random 125  ; Turn randomly by 45 to 170 degrees
    ]
  ]
end
...
to form-groups
  ...
  if any? others [
    ifelse any? others with [group-id != -1] [
      ; If nearby tortoises are in a group, join one
...
; Procedure for creating new group agents
to new-group
  foreach new-groups [
    [element] ->
    create-groups 1 [
      set tortoises-group element
      set num-agents count tortoises-group
      set id [group-id] of one-of tortoises-group
      set group-color [color] of one-of tortoises-group
      set time-created ticks
      let reference-tortoise one-of tortoises-group
      set xcor [xcor] of reference-tortoise
      set ycor [ycor] of reference-tortoise
      set shape "dot"
    ]
  ]
  set new-groups []
...
to update-groups
  foreach tortoises-to-add-to-group [
    [element] ->
    let element-id [group-id] of element
    ask groups with [id = element-id] [
      set tortoises-group (turtle-set element tortoises-group)
      set num-agents count tortoises-group
    ]
  ]
  set tortoises-to-add-to-group []
...
; Report the personality of a tortoise
to-report personality-giver
  ...
  ifelse (k < probability-of-generating-leader) [
    ...
    ifelse (k < probability-of-generating-participant + probability-of-generating-leader) [
...
; Show/hide group IDs based on the switch
to show-id
  ask groups [
...
; Show/hide group representations based on the switch
to show-groups
  ask groups [
    ifelse show-groups? [
      set size 3
